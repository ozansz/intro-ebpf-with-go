...
struct {
	...
	__type(key, u32); // source IPv4 address
...
*/
static __always_inline int parse_ip_src_addr(struct xdp_md *ctx, u32 *ip_src_addr) {
...
int xdp_prog_func(struct xdp_md *ctx) {
	...
	u32 *pkt_count = bpf_map_lookup_elem(&xdp_stats_map, &ip);
	if (!pkt_count) {
